...
interface VaultPickerModalProps {
  modelId: string;
  onSelect: (items: { label: string; thumbnailUrl?: string }[]) => void;
  onClose: () => void;
}

/**
 * Embeds the same live VNC connection OnlyFansViewer uses, navigated
 * straight to the model's real OnlyFans Vault - lets the admin browse and
 * click-select real files instead of guessing a search term blind. On
 * "Übernehmen", the VPS reads back whatever's currently selected in the
 * Vault's own multi-select UI and this passes that list up to the script
 * step being edited.
 */
export function VaultPickerModal({ modelId, onSelect, onClose }: VaultPickerModalProps) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        // Point this slot at the real Vault page before showing the feed.
  ...
  const handleConfirm = async () => {
    setIsCapturing(true);
    try {
      const res = await fetch("/api/crm/vault-picker/selection", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ modelId }),
      });
      ...
      <div className="w-full max-w-4xl bg-[#0A0A0A] border-2 border-[#C9A86A]/40 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-4 border-t border-[#9C7A3D]/20 flex items-center justify-between gap-3">
          <p className="text-[11px] text-slate-500">
            Im Tresor die gewünschten Dateien auswählen (Mehrfachauswahl möglich), dann übernehmen.
          </p>
          <button
