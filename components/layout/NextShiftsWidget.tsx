...
export default function NextShiftsWidget({
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-6 p-5 bg-gradient-to-r from-[#D4AF37]/15 to-[#8A6D3F]/10 border-2 border-[#D4AF37]/50 rounded-xl shadow-2xl relative overflow-hidden">
        {/* Animated background effect - subtle gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-r from-[#D4AF37]/5 via-transparent to-[#8A6D3F]/5 opacity-50 pointer-events-none"></div>
      ...
      {naechsteZweiSchichten.length === 0 ? (
        <div className="text-center py-8 px-6 bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-[#8A6D3F]/20 rounded-lg">
          ...
          <Link
            href="/management"
            className="inline-block px-6 py-2 bg-gradient-to-b from-[#D4AF37] to-[#8A6D3F] hover:from-[#E5C158] text-black font-bold rounded-lg text-xs uppercase tracking-wider transition-all hover:shadow-lg"
          >
            ⚙️ Zum Management
          </Link>
        </div>
        ...
        {naechsteZweiSchichten.map((shift, idx) => (
          <div key={shift.id} className="group relative bg-gradient-to-br from-[#1A1A0E] to-[#0A0A00] border-2 border-[#D4AF37]/30 rounded-lg p-5 hover:border-[#D4AF37]/70 hover:shadow-2xl hover:shadow-[#D4AF37]/20 transition-all duration-300 transform hover:scale-105">
            {/* Card Number Badge */}
            <div className="absolute top-0 right-0 w-12 h-12 bg-gradient-to-br from-[#D4AF37] to-[#8A6D3F] rounded-bl-lg flex items-center justify-center text-black font-black text-lg opacity-90 group-hover:opacity-100">
              {idx + 1}
            </div>
            ...
            <Link
              href="/chatter"
              className="w-full inline-block text-center bg-gradient-to-b from-[#D4AF37] to-[#8A6D3F] hover:from-[#E5C158] hover:to-[#BB8C21] text-black font-black py-3 px-3 rounded-lg text-xs uppercase tracking-wider transition-all transform hover:shadow-lg hover:shadow-[#D4AF37]/30 active:scale-95"
            >
