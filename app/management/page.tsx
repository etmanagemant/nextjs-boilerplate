...
export default async function ManagementPage() {
  ...
  return (
    <main className="p-6 max-w-4xl mx-auto min-h-screen bg-[#0A0A0A] text-[#F3E5AB] rounded-xl my-6 border border-[#8A6D3F]/20 shadow-2xl">
      <div className="flex justify-between items-center mb-6 border-b border-[#8A6D3F]/20 pb-4 flex-wrap gap-4">
        <div>
      ...
      <section className="bg-black/40 p-6 rounded-xl border border-[#8A6D3F]/10 mb-8 text-center shadow-lg">
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody>
              {sichereProfile.map((p) => (
                <tr key={p.user_id} className="border-b border-[#8A6D3F]/5 hover:bg-black/20 transition">
                  <td className="p-3">
                    <form action={updateMitarbeiterName} className="flex gap-2">
                      <input type="hidden" name="user_id" value={p.user_id} />
                      <input type="text" name="full_name" defaultValue={p.full_name || ""} required className="bg-[#050505] border border-[#8A6D3F]/30 rounded px-2 py-1 text-sm text-white focus:border-[#D4AF37] outline-none w-full max-w-[140px]" />
                  ...
                  <td className="p-3">
                    {p.email !== "etmanagement@gmail.com" && p.email !== "etmanagemant@gmail.com" && p.user_id !== "35498c92-2c4d-4720-a6f7-cc187a4c5fc4" ? (
                      <form action={updateMitarbeiterCompensation} className="flex gap-1.5 items-center flex-wrap">
                        ...
                        {p.role === "moderator" ? (
                          <>
                            <input type="number" step="0.01" name="hourly_rate" defaultValue={p.hourly_rate || 0} placeholder="EUR/h" className="w-20 bg-[#050505] border border-[#8A6D3F]/30 text-white rounded p-1 text-xs text-center outline-none focus:border-[#D4AF37]" />
                            <span className="text-[10px] text-slate-500">EUR/h</span>
                          </>
                        ) : (
                          <>
                            <input type="number" step="0.1" name="provision_rate" defaultValue={p.provision_rate || 20} placeholder="20" className="w-14 bg-[#050505] border border-[#8A6D3F]/30 text-white rounded p-1 text-xs text-center outline-none focus:border-[#D4AF37]" />
      ...
      <section className="bg-black/40 p-6 rounded-xl border border-[#8A6D3F]/10 shadow-lg">
        ...
        <form action={addModel} className="flex gap-3 mb-6">
          <input type="text" name="name" placeholder="Model Name" required className="flex-1 px-3 py-2 border border-[#8A6D3F]/30 rounded-md text-sm text-white bg-[#050505] focus:border-[#D4AF37] outline-none" />
          <button type="submit" className="bg-gradient-to-b from-[#D4AF37] to-[#8A6D3F] text-black px-4 py-2 rounded-md text-sm font-bold hover:from-[#E5C158] transition cursor-pointer">Model hinzufügen</button>
        </form>
