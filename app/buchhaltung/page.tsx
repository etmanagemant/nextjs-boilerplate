...
export default async function BuchhaltungPage() {
  ...
  return (
    <main className="p-6 max-w-5xl mx-auto min-h-screen bg-[#0A0A0A] text-[#F3E5AB] rounded-xl my-6 border border-[#8A6D3F]/20 shadow-2xl">
      ...
      <section className="bg-black/40 border border-[#8A6D3F]/10 p-4 rounded-xl mb-8">
        ...
        <form action={updateAgencySettings} className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <input type="text" name="agency_name" defaultValue={agency.agency_name} placeholder="Agentur Name" className="bg-[#050505] border border-[#8A6D3F]/30 rounded p-2 text-white outline-none" />
          <input type="text" name="tax_id" defaultValue={agency.tax_id || ""} placeholder="Umsatzsteuer-ID / Steuernummer" className="bg-[#050505] border border-[#8A6D3F]/30 rounded p-2 text-white outline-none" />
          <input type="text" name="address" defaultValue={agency.address || ""} placeholder="Firmenadresse" className="bg-[#050505] border border-[#8A6D3F]/30 rounded p-2 text-white outline-none" />
          <input type="text" name="bank_details" defaultValue={agency.bank_details || ""} placeholder="Zahlungsdaten / IBAN für Fußzeile" className="col-span-1 sm:col-span-3 bg-[#050505] border border-[#8A6D3F]/30 rounded p-2 text-white outline-none" />
          <div className="col-span-1 sm:col-span-3 flex justify-end"><button type="submit" className="bg-gradient-to-b from-[#D4AF37] to-[#8A6D3F] text-black px-4 py-1 rounded font-bold hover:from-[#E5C158] transition cursor-pointer">Stammdaten sichern</button></div>
        </form>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {chatterProfile.map((chatter) => {
            ...
            return (
              <div key={chatter.user_id} className="bg-black/30 border border-[#8A6D3F]/20 rounded-xl p-4 flex flex-col justify-between hover:border-[#D4AF37]/40 transition">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400 border-t border-[#8A6D3F]/5 pt-2 mt-2">
                ...
                <button className="w-full mt-4 bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 border border-[#8A6D3F]/30 text-[#D4AF37] rounded py-1 text-xs font-bold transition cursor-pointer">Abrechnung PDF laden</button>
              </div>
            );
          })}
        </div>
