...
/**
 * Proxies to the VPS's /of-massmessage-list - real OnlyFans mass message
 * history (last 30 days), same source as Statistik->Verlobung->Massen-
 * Nachrichten. CONFIRMED LIVE 2026-08-05.
 * GET ?modelId=X
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch(`/of-massmessage-list?modelId=${encodeURIComponent(modelId)}`);
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-MASSMESSAGE-LIST] Error:", error.message);
    return NextResponse.json({ error: error.message || "Failed to load mass message list" }, { status: 500 });
  }
}
