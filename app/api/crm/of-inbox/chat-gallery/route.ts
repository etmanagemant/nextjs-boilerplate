...
/**
 * Proxies to the VPS's /of-chat-gallery - the real Galerie button in a
 * chat's header, all media ever sent in that chat (Task #57).
 * GET /api/crm/of-inbox/chat-gallery?modelId=X&fanId=Y&offset=0
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch(`/of-chat-gallery?modelId=${encodeURIComponent(modelId)}&fanId=${encodeURIComponent(fanId)}${offset ? `&offset=${encodeURIComponent(offset)}` : ""}`);
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-CHAT-GALLERY] Error:", error.message);
