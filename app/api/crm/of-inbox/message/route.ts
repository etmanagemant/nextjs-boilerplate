...
/**
 * Proxies to the VPS's /of-message - deletes an own message within
 * OnlyFans' own 24h window (Task #56).
 * DELETE Body: { modelId, fanId, messageId }
 */
export async function DELETE(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch("/of-message", { method: "DELETE", body: JSON.stringify({ modelId, fanId, messageId }) });
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-MESSAGE-DELETE] Error:", error.message);
