...
/**
 * Proxies to the VPS's /of-stats - Massmessage-Statistik, letzte 30 Tage
 * (Task #48).
 * GET /api/crm/of-inbox/stats?modelId=X
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch(`/of-stats?modelId=${encodeURIComponent(modelId)}`);
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-STATS] Error:", error.message);
