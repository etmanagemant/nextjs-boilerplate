...
/**
 * Proxies to the VPS's /of-vault-media - Tresor-Medien, optional nach
 * Ordner gefiltert (Task #50).
 * GET /api/crm/of-inbox/vault-media?modelId=X&offset=0&listId=Y
 */
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[OF-INBOX-VAULT-MEDIA] Error:", error.message);
    return NextResponse.json({ error: error.message || "Failed to load vault media" }, { status: 500 });
  }
}
