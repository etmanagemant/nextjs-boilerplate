...
/**
 * Proxies to the VPS's /of-earnings - Kontostand/Auszahlungen.
 * GET /api/crm/of-inbox/earnings?modelId=X
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch(`/of-earnings?modelId=${encodeURIComponent(modelId)}`);
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-EARNINGS] Error:", error.message);
    return NextResponse.json({ error: error.message || "Failed to load earnings" }, { status: 500 });
  }
}
