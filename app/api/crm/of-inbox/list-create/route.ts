...
/**
 * Proxies to the VPS's /of-list-create - creates a new custom list.
 * CONFIRMED LIVE 2026-08-05 against a disposable test list.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch("/of-list-create", { method: "POST", body: JSON.stringify({ modelId, name }) });
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-LIST-CREATE] Error:", error.message);
