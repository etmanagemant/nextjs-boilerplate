...
/**
 * Proxies to the VPS's /of-vault-list-rename - benennt einen Tresor-Ordner
 * um. CONFIRMED LIVE 2026-08-06 via real network capture.
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const vpsRes = await vpsFetch("/of-vault-list-rename", { method: "PATCH", body: JSON.stringify({ modelId, listId, name }) });
    const data = await vpsRes.json();
    if (!vpsRes.ok) return NextResponse.json({ error: data.error || "VPS error" }, { status: vpsRes.status });
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("[OF-INBOX-VAULT-LIST-RENAME] Error:", error.message);
    return NextResponse.json({ error: error.message || "Failed to rename vault list" }, { status: 500 });
  }
}
