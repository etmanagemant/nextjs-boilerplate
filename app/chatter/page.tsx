...
export default async function ManagementPage() {
  ...
  // 🟢 FEHLER BEHOBEN: UUID- und E-Mail-Check strikt getrennt, um UUID-Konvertierungsfehler zu vermeiden
  let isAdmin = false;
  ...
  // Daten live aus deinen echten Tabellen laden
  ...
  const { data: geplanteShifts } = await supabase.from("shifts").select("*"); // Holt die geplanten Schichten

  const sichereProfile = profilListe || [];
  const sichereModels = modelsListe || [];
  const sichereSchichten = alleSchichten || [];
  const sichereGeplante = geplanteShifts || [];
  ...
  return (
    <main className="p-6 max-w-4xl mx-auto min-h-screen bg-slate-900 text-white rounded-lg my-6 border border-slate-800">
      ...
      {/* 🟢 NEU BEREICH: SCHICHTERSTELLUNG LOGIK */}
      <section className="bg-slate-950 p-6 rounded-lg border border-slate-800 mb-8 shadow-sm">
        ...
        <form action={addShift} className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Model zuteilen (Optional)</label>
          ...
          <div className="sm:col-span-2">
            <button type="submit" className="w-full bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-blue-700 transition">
