...
export default function ChatterPage() {
  ...
  // 1. Holt die E-Mail und die ID des eingeloggten Nutzers für den Datenschutz-Filter
  useEffect(() => {
  ...
  // 2. Refresh-Funktion mit striktem Datenschutz-Filter
  const refresh = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      // 🟢 DATENSCHUTZ-FILTER: Lädt nur Schichten, die zu DEINER E-Mail oder DEINER User-ID gehören!
  ...
  // Erstmaliges Laden der Daten (wartet, bis User-Daten da sind)
  useEffect(() => {
    if (!currentUserEmail && !currentUserId) return;
  ...
  async function triggerGlobalStart() {
    ...
    const { error } = await supabase
      ...
      .insert([
        {
          shift_id: 1, 
          chatter_id: currentUserEmail, // Speichert die Schicht direkt unter deiner E-Mail ab
      ...
      <div className="mt-6">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-white/60 border-t border-white/5 pt-2">
                    <div>
                      <span className="text-white/40">Nutzer:</span> <span className="text-blue-400 font-medium">{r.chatter_id.includes("@") ? r.chatter_id : "Dein Account"}</span>
                    </div>
                    <div>
            ...
            {rows.length === 0 && (
              <div className="text-sm text-slate-500 py-4 italic text-center border border-white/5 rounded bg-black/10">
                Noch keine Schichten auf diesem Account erfasst. Klicke oben auf "Start Schicht"!
              </div>
            )}
          </div>
        )}
